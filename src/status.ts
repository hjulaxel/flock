...
export function statusFacts(input: StatusInput): StatusFact[] {
  ...
  // The Codex pair, only where there is a Codex to hook — the gate the gear and
  // the checklist use. A row for an install that cannot run would be a line
  // that does nothing when picked, which the list promises never to hold.
  if (world.codexHooksAvailable === true) {
    facts.push(
      world.codexHooksInstalled === true
        ? {
            id: 'codexHooks',
            icon: 'plug',
            label: 'Codex hooks',
            value: 'installed',
            next: 'Remove them — a Codex row goes back to reading its rollout one poll late',
            action: { kind: 'command', command: COMMANDS.removeCodexHooks },
          }
        : {
            id: 'codexHooks',
            icon: 'plug',
            label: 'Codex hooks',
            value: 'not installed',
            next: 'Install them — a Codex row updates the moment an event happens',
            action: { kind: 'command', command: COMMANDS.installCodexHooks },
          },
    );
  }
