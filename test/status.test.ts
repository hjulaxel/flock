...
describe('status: the rows', () => {
  ...
  it('shows the Codex hooks only where there is a Codex to hook, one way round', () => {
    const none = statusFacts({ world: settled, cli, hasCodexAccount: false });
    expect(none.find((f) => f.id === 'codexHooks')).toBeUndefined();

    const off = statusFacts({
      world: { ...settled, codexHooksAvailable: true, codexHooksInstalled: false },
      cli,
      hasCodexAccount: true,
    });
    expect(byId(off, 'codexHooks').value).toBe('not installed');
    expect(byId(off, 'codexHooks').action).toEqual({
      kind: 'command',
      command: COMMANDS.installCodexHooks,
    });
    // Right after the Claude pair, before the verbs: the two hook rows read together.
    const ids = off.map((f) => f.id);
    expect(ids.indexOf('codexHooks')).toBe(ids.indexOf('hooks') + 1);
    expect(ids.indexOf('verbs')).toBe(ids.indexOf('codexHooks') + 1);

    const on = statusFacts({
      world: { ...settled, codexHooksAvailable: true, codexHooksInstalled: true },
      cli,
      hasCodexAccount: true,
    });
    expect(byId(on, 'codexHooks').value).toBe('installed');
    expect(byId(on, 'codexHooks').action).toEqual({
      kind: 'command',
